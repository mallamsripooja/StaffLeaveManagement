...
try:
    ...
    ht1='''
           ...
           <a href="stafflogin.py">Logout</a>&emsp;</td>
            ...
            <table align="center" height=55>
                <tr>
                    <td style="font-family:Tahoma;font-size:24px;">Half Day Leave</td>
                </tr></table>'''
    ht2='''<form method="POST">
                     ...
                     <tr>
                       <td>
                       <input type="radio" name="hr" required value=0 />Fore-noon</td>
                       <td align="center"><input type="radio" name="hr" reqired value=1 />After-noon</td>
                     </tr>
    ...
    if "sub2" in form:
        ...
        #print(data)
        cur.execute('select fdt from leave_tab where id='+str(id)+' and type="halfday" and approv=0 or approv=1')
        data1=cur.fetchall()
        #print(data)
        j=0
        ...
        for i in range(len(data[0])):
            if data[1][i]!='None':
                cur.execute('insert into leave_sub values(?,?,?,?,?,?)',(id,data1[0][0],data[1][i],data[2][j]+'-'+data[3][j]+'-'+data[4][j],data[0][i],approv))
                con.commit()
                j+=1
        ht2='''
            <table align="center" height=55>
    ...
    if "sub1" in form:
        ...
        if role=="co-ord":
            approv=1
        cur.execute('insert into leave_tab values(?,?,?,?,?,?,?,?,?,?)',(id,str(datetime.date.today()),'halfday',dt,dt,approv,'halfday',0,0,0.5))
        con.commit()
        h1='''
                <td><select name="yr">
