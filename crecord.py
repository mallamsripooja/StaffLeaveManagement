...
try:
    ...
    def weekday1(date):
        if(date==0):
            ...
            return 'Sunday'
    def day(date):
        cur.execute('select l.id,l.fdt,l.tdt,l.type from leave_tab l join staff_det s on s.id=l.id where approv=3 and tdt >="'+str(date)+'" and cord='+str(di))
        data=cur.fetchall()
        dte=date
        dte=str(dte).split('-')
...
'''
        ...
        for tup in data:
            ...
            h2=h2+h3
        if h2=='':
            h22=h22+'''<tr><td>Nobody is on leave</td></tr><br>'''
